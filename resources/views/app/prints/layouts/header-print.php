...
<!-- top btns -->
<div class="d-flex w400 m-auto bold">
  <a href="<?= url('sales') ?>">
    <div class="center mt10 border p20 btn">
      نمایش بِلهای فروش
    </div>
  </a>
  <div class="center mt10 border p20 btn">
  ...
  <div class="center mt10 border p20 btn" onclick="printReceipt()">
    چاپ مجدد
  </div>
</div>
  ...
  <div class="border p5">
    ...
    <div class="d-flex justify-between pr">
      ...
      <div class="top-desc-one mt5">
        <div class="fs15 color-print">نام خریدار: <?= (isset($invoice_infos['user_name']) && $invoice_infos) ? $invoice_infos['user_name'] : 'عمومی' ?></div>
        <div class="fs14 color-print">
          شماره موبایل:
          <?= htmlspecialchars(
            trim($invoice_infos['phone'] ?? '') !== '' ? $invoice_infos['phone'] : 'ثبت نشده',
            ENT_QUOTES,
            'UTF-8'
          ) ?>
        </div>
        <div class="fs14 color-print">
          آدرس:
          <?= htmlspecialchars(
            trim($invoice_infos['address'] ?? '') !== '' ? $invoice_infos['address'] : 'ثبت نشده',
            ENT_QUOTES,
            'UTF-8'
          ) ?>
        </div>
      ...
      <div class="qrcode-sale">
        <svg id="qrcode"></svg>
      </div>

      <div class="top-desc-one mt5 center">
        <div class="fs15 color-print bold">شماره بِل: <?= $this->to_farsi_number($invoice_infos['invoice_number']) ?></div>
        <div class="fs15 color-print">تاریخ: <?= jdate('Y/m/d', $invoice_infos['date']) ?> <span class="fs11">(<?= jdate('H:i', $invoice_infos['date']) ?>)</span>
        </div>
        <div class="fs15 color-print">توسط: <?= $invoice_infos['who_it'] ?></div>
      </div>
